...
function validateRequestData(client: ClientData | undefined) {
  ...
  console.log(
    `Creating folder for client type: ${client.person_type}, name: ${folderName}`
  );
...
function validateGoogleDriveCredentials() {
  ...
  const googlePrivateKey = Deno.env
    .get("GOOGLE_PRIVATE_KEY")
    ?.replace(/\\n/g, "\n");
  const rootFolderId = Deno.env.get("GOOGLE_DRIVE_ROOT_FOLDER_ID");

  // >>> DEBUG: verifique se está vindo corretamente
  console.log(
    "GOOGLE_PRIVATE_KEY sample (start):",
    googlePrivateKey?.slice(0, 100)
  );
  console.log(
    "GOOGLE_PRIVATE_KEY sample (end):",
    googlePrivateKey?.slice(-100)
  );
  console.log("GOOGLE_PRIVATE_KEY length:", googlePrivateKey?.length);

  // Log credential availability and details for debugging
  console.log("Google Drive credentials:", {
    clientEmail: googleClientEmail
      ? `${googleClientEmail.substring(0, 5)}...`
      : "Missing",
...
async function initializeGoogleDrive(
  ...
  try {
    ...
    throw new Error(
      `Erro ao inicializar API do Google Drive: ${driveError.message}`
    );
  }
}
...
async function verifyRootFolder(drive: any, rootFolderId: string | undefined) {
  if (rootFolderId) {
    try {
      ...
      console.log(
        `Root folder verified: ${rootFolder.data.name} (${rootFolder.data.id})`
      );
      return true;
    } catch (rootFolderError) {
      console.error("Error accessing root folder:", rootFolderError);
      throw new Error(
        `Cannot access root folder with ID ${rootFolderId}. Check permissions or if folder exists.`
      );
    }
  }

  console.log(
    "Root folder ID not set in environment, using default behavior (creating at Drive root)"
  );
  return true;
}
...
async function createMainFolder(
  ...
  try {
    ...
    console.error(
      ...
      JSON.stringify(folderError, null, 2)
    );
    throw new Error(
      `Failed to create main folder: ${folderError.message || "Unknown error"}`
    );
  }
}
...
async function updateClientWithFolderId(
  supabase: any,
  clientId: string,
  mainFolderId: string
) {
  try {
    ...
    console.log(
      `Updated client record ${clientId} with folder ID ${mainFolderId}`
    );
  } catch (updateErr) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!credentialsResult.valid) {
      return new Response(JSON.stringify({ error: credentialsResult.error }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 500,
      });
    }
    const { googleClientEmail, googlePrivateKey, rootFolderId } =
      credentialsResult;
